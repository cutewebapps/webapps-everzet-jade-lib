...
class Everzet_Jade_Jade
{
     ...
     * Initialize parser. 
     * 
     * @param   Everzet_Jade_Lexer_LexerInterface  $lexer  jade lexer
     */
    public function __construct(
                    Everzet_Jade_Parser $parser, 
                    Everzet_Jade_Dumper_DumperInterface $dumper, 
                    $cache = null)
    {
        $this->parser = $parser;
        $this->dumper = $dumper;
    ...
    public function cache($input)
    {
        if (null === $this->cache || !is_dir($this->cache)) {
            throw new Everzet_Jade_Exception('You must provide correct cache path to Jade for caching.');
        }
        ...
        if (!is_writable($this->cache)) {
            throw new Everzet_Jade_Exception(sprintf('Cache directory must be writable. "%s" is not.', $this->cache));
        }
    ...
    {
        if (is_file($input)) {
            if ( substr( $input, 0, strlen( CWA_APPLICATION_DIR ) ) == CWA_APPLICATION_DIR ) {
                $strPath = substr( $input, 1 + strlen( CWA_APPLICATION_DIR ));
                $strPath = str_replace( '/', '__', $strPath );
                return $strPath;
            } else 
                return basename($input, '.jade');
        } else {
            throw new InvalidArgumentException('Only file templates can be cached.' .$input );
        }
    }
