 ...
 * Jade Lexer. 
 */
class Everzet_Jade_Lexer_Lexer implements Everzet_Jade_Lexer_LexerInterface
{
    protected $input;
    ...
    protected function scanIndentation()
    {
        $matches = array();
        $nIndentSize = 4;
        $objJadeConfig = App_Application::getInstance()->getConfig()->jade;
        if ( is_object( $objJadeConfig ) && $objJadeConfig->indentation )
            $nIndentSize = intval( $objJadeConfig->indentation );
        ...
        if (preg_match('/^\n( *)/', $this->input, $matches)) {
            ...
            $indents    = mb_strlen($token->value) / $nIndentSize;
            ...
            if (mb_strlen($this->input) && "\n" === $this->input[0]) {
                ...
                throw new Everzet_Jade_Exception(sprintf(
                    'Row #'.$this->lineno.': Invalid indentation found. Spaces count must be a multiple of two, but %d got.'
                ...
                throw new Everzet_Jade_Exception(sprintf(
                    'Row #'.$this->lineno.': Invalid indentation found. Got %d, but expected %d '
                  , $indents
                  , $this->lastIndents + 1
                ));
            } elseif ($this->lastIndents > $indents) {
